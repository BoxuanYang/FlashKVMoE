...
# Adapted routing/MLA from KT archive/ktransformers/models/modeling_deepseek_v3.py.
...
class DeepseekRouter(BaseOP):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        choice = grouped.masked_fill(~mask.unsqueeze(-1), -torch.inf).flatten(1)
...
class DeepseekMoEWrapper(BaseOP):
    def __init__(self, config: ModelConfig):
...
class DeepseekV3ForCausalLM(BaseLLMModel):
    def __init__(self, config: ModelConfig):
