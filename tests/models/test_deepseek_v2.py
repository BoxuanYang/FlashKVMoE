...
from minisgl.models.deepseek_v2 import DeepseekMLA, DeepseekMLP, DeepseekMoE, DeepseekRouter
...
def test_hf_config_loads_without_remote_code(tmp_path):
    from minisgl.utils.hf import cached_load_hf_config
    from transformers import DeepseekV2Config

    DeepseekV2Config(**vars(hf_config())).save_pretrained(tmp_path)
    config = ModelConfig.from_hf(cached_load_hf_config(str(tmp_path)))
    assert config.is_moe and config.is_mla
    assert config.num_experts == 8 and config.head_dim == 192
    assert config.rotary_config.scaling["rope_type"] == "yarn"
    assert config.routed_scaling_factor == 16.0


@pytest.fixture(scope="module")
def archive():
    path = Path(__file__).resolve().parents[2] / (
        "third_party/ktransformers/archive/ktransformers/models/modeling_deepseek.py"
    )
    names = {
        "MoEGate",
        "DeepseekV2RMSNorm",
    ...
    namespace = {
        ...
        "Tuple": Tuple,
        "Cache": object,
        "DeepseekV2Config": SimpleNamespace,
    }
...
def test_router_matches_archive(config, archive, tokens, normalize):
    ...
    ref = archive.MoEGate(hf_config(norm_topk_prob=normalize)).eval()
    ...
    expected = ref(x.unsqueeze(0))[:2]
    for a, b in zip(actual, expected):
        torch.testing.assert_close(a, b, rtol=0, atol=0)


def test_moe_calls_kt_directly(config, monkeypatch):
    layer = DeepseekMoE(config)
    x = torch.randn(3, config.hidden_size)
    layer.gate.weight.normal_(std=0.03)
    ids, weights = layer.gate.forward(x)
    monkeypatch.setattr(
        torch.cuda, "current_stream", lambda device: SimpleNamespace(cuda_stream=71)
    )

    def routed(actual_x, actual_ids, actual_weights, stream):
    ...
    layer._wrapper = SimpleNamespace(forward=routed)
    layer.shared_experts = SimpleNamespace(forward=lambda actual_x: actual_x * 2)
    torch.testing.assert_close(layer.forward(x), x * 5)


def write_v2(path, c, split=False, compressed_kv=True):
...
def test_legacy_v2_gguf_metadata(config, tmp_path):
    path = tmp_path / "v2.gguf"
    write_v2(path, config, compressed_kv=False)
...
def test_gguf_and_model_state(config, tmp_path, monkeypatch, split, byte_split):
    path = tmp_path / "v2.gguf"
    tensors = write_v2(path, config, split)
    if byte_split:
        ...
        for i in range(count):
            (path / f"v2.gguf.part{i + 1}of{count}").write_bytes(
                content[i * part_size : (i + 1) * part_size]
            )
    loader = GGUFWeights(str(path), config)
    cache = {}
    kt_loader = SimpleNamespace(_gguf_loaders_by_path=cache)

    def create_wrapper(**kwargs):
        injected = cache[os.path.realpath(kwargs["weight_path"])]
    ...
    monkeypatch.setattr("minisgl.models.deepseek_v2.get_rope", lambda *args: None)
    ...
    assert isinstance(model.model.layers.op_list[1].mlp, DeepseekMoE)
...
def test_v2_cache_storage_and_budget(config, monkeypatch):
...
def test_full_v2_gpu_weight_budget(config, monkeypatch):
    ...
    assert sum(isinstance(l.mlp, DeepseekMLP) for l in model.model.layers.op_list) == 1
    assert sum(isinstance(l.mlp, DeepseekMoE) for l in model.model.layers.op_list) == 59
...
def test_yarn_frequencies_match_archive(config, archive, monkeypatch):
    ...
    ref = archive.DeepseekV2YarnRotaryEmbedding(
        64, 8192, scaling_factor=40, mscale=1, mscale_all_dim=1
    )
    cos, sin = ref(torch.empty(1, dtype=torch.float32), torch.arange(8192)[None])
    torch.testing.assert_close(rope._cos_sin_cache[:, :32], cos[0, :, :32], atol=0.002, rtol=0.002)
    torch.testing.assert_close(rope._cos_sin_cache[:, 32:], sin[0, :, :32], atol=0.002, rtol=0.002)
...
def test_mla_against_archive_attention(config, archive, tmp_path, monkeypatch, device_name):
    ...
    if device_name == "cpu":
        ...
        from minisgl.models.deepseek_v2 import yarn_mscale
    ...
    ref = (
        archive.DeepseekV2Attention(hf, layer_idx=0).to(device=device, dtype=torch.bfloat16).eval()
    )
    ref_state = {}
    for name, source in {
        "q_a_proj": "attn_q_a",
